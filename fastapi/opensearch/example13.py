...
class Repository:
    ...
    def get_data(self, index: str, doc_id: str) -> Optional[dict]:
        """
        Retrieves a document by ID from the specified OpenSearch index.
        """
        return self.client.get(index=index, id=doc_id, ignore=[404])

    def search_logs(self, index: str) -> list:
        """
        Retrieves all log entries from the specified OpenSearch index.
        """
        response = self.client.search(index=index, body={"query": {"match_all": {}}})
...
async def handle_data(data: DataModel, user: str):
    ...
    try:
        ...
        existing_doc = repository.get_data(index=DATA_INDEX, doc_id=doc_id)
...
async def get_data(doc_id: str):
    ...
    """
    try:
        doc = repository.get_data(index=DATA_INDEX, doc_id=doc_id)
...
async def get_logs():
    ...
    """
    try:
        logs = repository.search_logs(index=LOGS_INDEX)
        return logs
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
